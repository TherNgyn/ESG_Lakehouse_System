...
with scope_emissions as (
    ...
    from {{ ref('fact_esg_metric') }} f
    join {{ ref('dim_company') }} c on f.company_id = c.company_key
    join {{ ref('dim_metric') }} m on f.metric_id = m.metric_key
    where m.metric_name LIKE 'Scope 1 & 2 Emissions'
        and f.year is not null
        and f.normalized_value is not null
    {% if is_incremental() %}
        and f.year >= (select coalesce(max(year), 1900) - 2 from {{ this }})
    {% endif %}
...
employee_turnover as (
    ...
    where m.metric_name LIKE 'Employee Turnover Rate - Total'
        ...
        and f.normalized_value is not null
    {% if is_incremental() %}
        and f.year >= (select coalesce(max(year), 1900) - 1 from {{ this }})
    {% endif %}
    group by f.company_id, c.industry_normalized, c.sector_normalized, f.year
),
...
{% if is_incremental() %}
where year >= (select coalesce(max(year), 1900) - 1 from {{ this }})
{% endif %}